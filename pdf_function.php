...
class setpdf {
    ...
    function __construct($guid, $connection2) {
        ...
        if ($this->rollGroupID > 0) {
            ...
            $this->rollGroupList = $this->readRollGroupList($connection2, $this->rollGroupID, $this->showLeft);
        }
        /*
        $this->repAccess = 0;
        if ($this->reportID > 0) {
            $this->repAccess = $this->findRepAccess($guid, $connection2);
        }
    ...
    ////////////////////////////////////////////////////////////////////////////
    function findRepAccess($guid, $connection2) {
        // check if use should have editing access to the reports

        // check if administrator
        $admin = read_access($connection2, 'admin', $_SESSION[$guid]["gibbonPersonID"]);

        //   or slt
        $slt = read_access($connection2, 'senior', $_SESSION[$guid]["gibbonPersonID"]);
