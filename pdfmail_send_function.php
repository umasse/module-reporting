...
class setpdf {
    ...
    ////////////////////////////////////////////////////////////////////////////
    function readLastStudentReportFilename() {
        $data = array(
            'studentID' => $this->studentID,
        ...
        // check rows returned is more than 0
        // if so, return reportName
        if ($rs->rowCount() > 0) {
            $row = $rs->fetch();
            $reportFilename = $row['reportName'];
        }
    ...
    function checkFolder() {
        ...
        $path = $_SERVER['DOCUMENT_ROOT'].'/archive';
        if (!is_dir($path)) {
            mkdir($path);
        }
        makeIndex($path);
